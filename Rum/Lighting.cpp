...
Color<double>::type Lighting::BlinnPhong(IObject* obj, const Vector4D &hit_pos, const Vector4D & viewer, const Vector4D& n) const
 {
    ...
    for (auto it = mLights.begin(); it != mLights.end(); ++it)
    {
        ...
        tcol = diffuse * modDiffuse + m.specular * (i * it->mPower / ldst);

        if (m.doSSS)
        {
            tcol = tcol + m.sss * SubSurfaceScattering(hit_pos, viewDir * -1.0, 100, 7, 0.04);
        }
        ...
        if (mSoftShadow)
        {
            tcol = tcol * SoftShadow(hit_pos, lDir, ldst, it->mPower);
        }

        // blend color from different lights (additive model) with optional AO and SSS
        color = color + tcol;
            //((diffuse * modDiffuse + m.sss * (m.doSSS? m.sss * SubSurfaceScattering(hit_pos, viewDir * -1.0) : 1.)) + (m.specular * (i * it->mPower / ldst))) * (mSoftShadow? SoftShadow(hit_pos, lDir, ldst, it->mPower, 1.25, 0.001) : 1.0);
    }
