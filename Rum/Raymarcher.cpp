...
Color<double>::type Raymarcher::TraceColor(Ray ray) const
{
    const double  maxDst = m_maxdist;
    const double epsilon = m_epsilon;
    const int   maxSteps = m_maxsteps;

    if (m_scene == nullptr) return NewPoint4D(0,0,0,0);
    ...
    do
    {
        ...
        if (dst < epsilon) 
        {
            ray += dst;
            location = ray.GetEnd();  location[3] = 1;
            Color<double>::type color = m_light->ComputeLight(hit.first, location, hit.first->Normal(location), ray.mOrigin);
